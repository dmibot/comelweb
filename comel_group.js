...
module.exports = async (comel, m) => {
    if (m) {
        let contact = await comel.getContactById(m.author)
        let nameUser = contact.pushname || contact.shortName || contact.name
        let metadata = await comel.getChatById(m.id.remote)
...
⬡ *New Description :*\n${m.body}
`
           comel.sendMessage(m.id.remote, teks, { mentions: [contact] })
...
⬡ *Type :* ${m.body}
`
           let pp = comel.getProfilePicUrl(m.id.remote) || 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
           let message = await MessageMedia.fromUrl(pp)
           comel.sendMessage(m.id.remote, message, { caption: teks, mentions: [contact] })
...
`
           ...
           let mentions = []
           for (let jid of participants) mentions.push(await comel.getContactById(jid))
           let teks = `
...
`
           comel.sendMessage(m.id.remote, teks, { mentions })
...
`
           ...
           for (let jid of participants) {
               // get contact
               let contact = await comel.getContactById(jid)
               // get profile url user
               let ppuser = await comel.getProfilePicUrl(jid) || 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
               // to message
               let message = await MessageMedia.fromUrl(ppuser)
               let teks = '「 *Promote Deteᴄted* 」\n\n```User : @user```\n```Name : @name```\n```Group : @subject```\n\nHas Become An Admin'
               comel.sendMessage(m.id.remote, message, { caption: teks.replace(/@subject/g, metadata.name).replace(/@user/g, `@${jid.split("@")[0]}`).replace(/@name/g, `${contact.pushname || contact.shortName || contact.name}`).replace(/@date/g, `${new Date(m.timestamp * 1000)}`), mentions: [contact] })
           }
